...
function supabaseErrorToHttpError(action, table, error, fallbackMessage = 'Supabase operation failed') {
  ...
  const schemaCacheError = rawMessage.toLowerCase().includes('schema cache');
  const missingColumn = rawMessage.toLowerCase().includes('could not find') && rawMessage.toLowerCase().includes('column');
  const code = schemaCacheError || missingColumn ? 'SUPABASE_SCHEMA_CACHE_MISMATCH' : 'SUPABASE_ERROR';
  const message = code === 'SUPABASE_SCHEMA_CACHE_MISMATCH'
