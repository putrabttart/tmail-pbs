    ...
    if (expiryDate && expiryDate - now < 5 * 60 * 1000) {
      try {
        ...
        if (refreshErr.message && refreshErr.message.includes('invalid_grant')) {
          log('error', 'Refresh token revoked or expired — re-authentication required');
  ...
  if (USE_SUPABASE_STORAGE) {
    ...
    if (existing) {
      ...
      if (Object.keys(updates).length) {
        await client.from(SUPABASE_TABLE_ALIASES).update(updates).eq('address', addr);
      }
    } else {
  ...
  if (USE_SUPABASE_STORAGE) {
    ...
    if (client) {
      ...
      if (pinError) {
        ...
        throw new HttpError(500, 'Gagal menyimpan PIN: ' + pinError.message);
      }
