...
const envSchema = z.object({
  ...
  MESSAGE_CACHE_TTL_MS: z.string().optional(),
...
const MESSAGE_CACHE_TTL_MS = Math.min(
  Math.max(parseInt(env.MESSAGE_CACHE_TTL_MS || '180000', 10) || 180000, 60000),
  600000
);
...
const CACHE_TTL_MS = MESSAGE_CACHE_TTL_MS;
const messageMetaInflight = new Map();
const messageDetailInflight = new Map();

async function runDeduped(map, key, producer) {
  const existing = map.get(key);
  if (existing) return existing;

  const task = Promise.resolve().then(producer);
  map.set(key, task);
  try {
    return await task;
  } finally {
...
async function listMessages(alias, options = {}) {
  ...
  const results = (await Promise.all(
    messages.map(async (msg) => {
      ...
      return runDeduped(messageMetaInflight, cacheKey, async () => {
        const cachedRetry = cacheGet(cacheKey);
...
async function getMessageDetail(id) {
  ...
  const detailCacheKey = `detail:${id}`;
  const cached = cacheGet(detailCacheKey);
  if (cached) return cached;

  return runDeduped(messageDetailInflight, detailCacheKey, async () => {
    const cachedRetry = cacheGet(detailCacheKey);
    if (cachedRetry) return cachedRetry;
